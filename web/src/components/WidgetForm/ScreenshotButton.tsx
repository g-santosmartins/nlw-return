...
interface ScreenshotButtonProps {
    ...
    onScreenshotTook: (base64image: string | null) => void
}
...
export function ScreenshotButton({ onScreenshotTook, screenshot }: ScreenshotButtonProps) {
    ...
    // waits until it's done with async
    async function handleTakeScreenshot() {
        setIstakingScreenshot(true)

        // take a print of the screen using html2canva
        const canvas = await html2canvas(document.querySelector('html')!)

        // convert it into a base 64 url image output: png
